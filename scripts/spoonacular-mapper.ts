...
import type { Recipe, RecipeIngredientRequirement, RecipePreflightRequirement } from '../src/domain/types';
...
export interface SpoonacularExtractResponse {
  id: number;
  title: string;
...
export function mapSpoonacularToVibeMeals(data: SpoonacularExtractResponse): Recipe {
  // Extract ingredients
  const ingredients = (data.extendedIngredients || []).map(ing => 
    mapIngredient(ing, data.title)
  );
  ...
  let leftoverStrategy: 'NONE' | 'EXPECTED' | 'COOK_ONCE_EAT_TWICE' = 'NONE';
  if (servings >= 10) {
...
function inferCriticality(displayName: string, recipeName: string): 'CRITICAL' | 'NON_CRITICAL' {
...
function inferKind(displayName: string): RecipeIngredientRequirement['kind'] {
  const lower = displayName.toLowerCase();
...
function inferShoppingCategory(
  displayName: string,
  aisle?: string
): RecipeIngredientRequirement['shoppingCategory'] {
  const lower = displayName.toLowerCase();
  
  // Use Spoonacular aisle if available
  if (aisle) {
    const aisleLower = aisle.toLowerCase();
    if (aisleLower.includes('meat') || aisleLower.includes('seafood')) return 'MEAT_SEAFOOD';
    if (aisleLower.includes('produce')) return 'PRODUCE';
    if (aisleLower.includes('dairy') || aisleLower.includes('egg')) return 'DAIRY_EGGS';
    if (aisleLower.includes('frozen')) return 'FROZEN';
...
function extractSteps(data: SpoonacularExtractResponse) {
  const steps: Array<{ stepNumber: number; instruction: string }> = [];
...
function detectPreflight(
  data: SpoonacularExtractResponse,
  ...
  // Detect THAW
  const hasFrozenProtein = ingredients.some(ing => 
    ing.displayName.toLowerCase().match(/frozen.*(?:chicken|beef|fish|shrimp)/)
  );
...
function extractTags(
  ...
  if (title.match(/one pot|one-pot|one pan/)) tags.push('one_pot');
  if (title.match(/sheet pan|sheet-pan|baking sheet/)) tags.push('sheet_pan');
  if (title.match(/slow cooker|crockpot/)) tags.push('slow_cooker');
  if (title.match(/instant pot|pressure cooker/)) tags.push('instant_pot');
  if (title.match(/batch|big batch/)) tags.push('batch_cook');
  
  // Time-based tags
  if (timeBand === 'FAST') tags.push('quick');
  if (preflight.some(p => p.type === 'MARINATE' || p.type === 'SLOW_COOK' || p.type === 'LONG_PREP')) {
    tags.push('make_ahead');
  }
  
  // Vibe tags
  if (allText.match(/comfort|cozy|hearty/)) tags.push('comfort_food');
  if (title.match(/easy|simple|quick|weeknight/) || timeBand === 'FAST') tags.push('kid_friendly');
  if (title.match(/budget|cheap|frugal/) || data.sourceUrl?.includes('budgetbytes')) {
    tags.push('budget_friendly');
  }
  if (allText.match(/healthy|nutritious|wholesome/)) tags.push('healthy');
  if (title.match(/meal prep/)) tags.push('meal_prep');
...
function detectEquipment(data: SpoonacularExtractResponse, steps: Array<{ instruction: string }>): string[] {
  const equipment: string[] = [];
  const allText = [data.title, ...steps.map(s => s.instruction)].join(' ').toLowerCase();
  
  if (allText.match(/dutch oven/)) equipment.push('DUTCH_OVEN');
  if (allText.match(/sheet pan|baking sheet/)) equipment.push('SHEET_PAN');
  if (allText.match(/slow cooker|crockpot|crock pot/)) equipment.push('SLOW_COOKER');
  if (allText.match(/instant pot|pressure cooker/)) equipment.push('INSTANT_POT');
  if (allText.match(/blender/)) equipment.push('BLENDER');
  if (allText.match(/food processor/)) equipment.push('FOOD_PROCESSOR');
  if (allText.match(/grill|grilling/)) equipment.push('GRILL');
