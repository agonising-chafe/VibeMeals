...
import { mapSpoonacularToVibeMeals } from './spoonacular-mapper';
// ============================================================================
// Configuration
// ============================================================================
...
// ============================================================================
async function trySpoonacularExtract(url) {
    if (!SPOONACULAR_KEY)
        return null;
    ...
    try {
        ...
        const ingredients = data.extendedIngredients?.map((ing) => ing.original || ing.originalString).filter(Boolean) || [];
        const instructionsArray = (data.analyzedInstructions?.[0]?.steps || []).map((step) => step.step).filter(Boolean);
        const fallbackInstructions = Array.isArray(data.instructions)
            ? data.instructions
...
// Scraping Functions
// ============================================================================
function getPrintUrlVariants(url) {
...
async function scrapeRecipeSchema(url) {
...
function parseRecipeSchemaFromHtml(html) {
...
function extractFromPlainText(text) {
    ...
    for (const rawLine of lines) {
        ...
        if (mode === 'inst') {
            ...
            if ((upper === line && line.length < 30) || upper === 'DUTCH OVEN')
                continue;
            ...
            if (ingredients.length > 0 && (line.match(/^(melt|stir|add|cook|heat|bring|remove|serve|season|reduce|simmer|dice|chop)/i) ||
                line.length > 20)) {
...
// AI Enhancement
// ============================================================================
async function enhanceWithAI(schema, ingredients) {
    if (!GEMINI_API_KEY) {
...
function buildEnhancementPrompt(schema, ingredients) {
...
async function interactiveReview(schema, ingredients, aiEnhancement, estimatedMinutes) {
    console.log('\n' + '='.repeat(60));
...
function suggestCriticality(displayName) {
...
function suggestIngredientKind(displayName) {
    const lower = displayName.toLowerCase();
    if (lower.match(/chicken|beef|pork|fish|salmon|shrimp|tofu|beans|lentils/)) {
        return 'PROTEIN';
    }
...
function suggestShoppingCategory(displayName) {
    const lower = displayName.toLowerCase();
...
// Save Functions
// ============================================================================
async function saveRecipe(recipe, sourceUrl) {
