...
// Group by time band
const byBand = { FAST: [] as RecipeAnalysis[], NORMAL: [] as RecipeAnalysis[], PROJECT: [] as RecipeAnalysis[] };
recipes.forEach(r => {
  if (byBand[r.timeBand as keyof typeof byBand]) {
    byBand[r.timeBand as keyof typeof byBand].push(r);
  }
});
