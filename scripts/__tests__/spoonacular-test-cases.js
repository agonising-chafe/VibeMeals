...
const cauliflowerTacosResponse = {
    id: 716627,
...
allCases.forEach(({ name, data }, i) => {
    try {
        ...
        if (recipe.ingredients.length === 0)
            warnings.push('No ingredients');
        if (recipe.steps.length === 0)
            warnings.push('No steps');
        if (!recipe.name)
