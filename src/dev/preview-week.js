...
function printShoppingList() {
    ...
    for (const item of shoppingList.items) {
        ...
        console.log(`${critical} ${item.displayName} – ${item.totalAmount} ${item.unit} [${item.shoppingCategory}]`);
...
function printTonightState(date, missingItems = []) {
    const plan = buildDemoPlan();
    const tonight = computeTonightState(plan, seedRecipes, missingItems, [], date, new Date());
    console.log(`\n=== TONIGHT STATE – ${date} ===`);
    console.log(`Status: ${tonight.status}`);
    console.log(`Message: ${tonight.primaryMessage}`);
    if (tonight.secondaryMessage)
        console.log(`Details: ${tonight.secondaryMessage}`);
    ...
    if (tonight.tomorrowPreview) {
        ...
        console.log(`Tomorrow: ${t.date} – ${t.recipeName ?? 'nothing planned'} ` +
...
function main() {
    ...
    const preflightState = computeTonightState(planForPreflight, seedRecipes, [], [], '2025-01-10', new Date());
