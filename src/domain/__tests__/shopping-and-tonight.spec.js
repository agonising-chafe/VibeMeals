...
function makePlannedDinner(recipeId) {
    return {
        recipeId,
        servings: 4,
        locked: false,
...
function makePlanWithDinners(dinnersByDate) {
...
function getRecipeById(id) {
    ...
    if (!found)
        throw new Error(`Recipe not found in seedRecipes: ${id}`);
    return found;
}
describe('Shop domain – buildShoppingList', () => {
    it('includes all CRITICAL ingredients from planned dinners', () => {
        ...
        const criticalIds = new Set(recipes.flatMap(r => r.ingredients
        ...
        for (const ingredientId of criticalIds) {
            const match = shoppingList.items.find(item => item.ingredientId === ingredientId);
            expect(match, `Expected ShoppingList to include critical ingredientId=${ingredientId}`).toBeDefined();
        }
    });
    it('only offers NON_CRITICAL items as Quick Review candidates', () => {
        ...
        const ingredientCriticality = new Map();
        ...
        for (const candidate of shoppingList.quickReviewCandidates) {
            ...
            expect(crit, `Quick Review should only show NON_CRITICAL ingredients, but found ${item.ingredientId} with criticality=${crit}`).toBe('NON_CRITICAL');
        }
    });
});
describe('Today domain – computeTonightState', () => {
    it('returns READY when tonight has a planned dinner, all good preflight, and no missing items', () => {
        ...
        const tonightState = computeTonightState(plan, seedRecipes, [], // missingItems
        [], // substitutions
        '2025-01-06', new Date('2025-01-06T17:00:00Z'));
        expect(tonightState.status).toBe('READY');
    ...
    it('returns MISSED_PREFLIGHT when tonight has unmet preflight (e.g., marinade not done)', () => {
        ...
        [], // substitutions
        '2025-01-06', new Date('2025-01-06T17:00:00Z'));
        expect(tonightState.status).toBe('MISSED_PREFLIGHT');
    ...
    it('returns MISSING_INGREDIENT when a core ingredient is missing for tonight', () => {
        ...
        if (!criticalIng)
            throw new Error('Expected a critical ingredient');
        ...
        const tonightState = computeTonightState(plan, seedRecipes, missingItems, [], '2025-01-06', new Date('2025-01-06T17:00:00Z'));
        expect(tonightState.status).toBe('MISSING_INGREDIENT');
        expect(tonightState.issues.missingCoreIngredients.some(i => i.ingredientId === criticalIng.ingredientId)).toBe(true);
        expect(tonightState.actions.canStartCooking).toBe(false);
    });
    it('allows cooking when only NON_CRITICAL ingredients are missing', () => {
        ...
        if (!nonCritical)
            throw new Error('Expected a non-critical ingredient');
        const item = shoppingList.items.find(i => i.ingredientId === nonCritical.ingredientId);
        if (!item)
            throw new Error('Expected ShoppingList item for non-critical ingredient');
        const missingItems = [
            {
                id: 'missing_nc',
                shoppingItemId: item.id,
        ...
        expect(tonightState.status).toBe('READY');
        expect(tonightState.actions.canStartCooking).toBe(true);
        expect(tonightState.issues.missingNonCriticalIngredients.some(i => i.ingredientId === nonCritical.ingredientId)).toBe(true);
...
describe('Shop domain – quick review safety', () => {
    it('never surfaces PROTEIN ingredients in Quick Review', () => {
        ...
        const shoppingList = buildShoppingList(plan, seedRecipes, HOUSEHOLD);
        const ingredientKind = new Map();
        for (const recipe of seedRecipes) {
            for (const ing of recipe.ingredients) {
                ingredientKind.set(ing.ingredientId, ing.kind);
            }
        }
        for (const candidate of shoppingList.quickReviewCandidates) {
            const item = shoppingList.items.find(i => i.id === candidate.shoppingItemId);
            ...
            expect(kind, `Quick Review must not include PROTEIN ingredients (found ${item.ingredientId})`).not.toBe('PROTEIN');
        }
    });
});
