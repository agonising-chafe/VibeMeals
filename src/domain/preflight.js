 ...
 */
export function hoursUntilCook(cookDate, now = new Date()) {
 ...
 */
function checkRequirementMet(requirement, cookDate, now = new Date()) {
 ...
 */
export function detectPreflightStatus(recipe, cookDate, now = new Date()) {
 ...
 */
export function describePreflightRequirements(recipe) {
    if (!recipe.preflight || recipe.preflight.length === 0) {
        return [];
    }
 ...
 */
export function getAtRiskRequirements(recipe, cookDate, hoursWarningThreshold = 2, now = new Date()) {
    if (!recipe.preflight || recipe.preflight.length === 0) {
        return [];
    }
    const hoursRemaining = hoursUntilCook(cookDate, now);
...
export function summarizePreflightStatus(recipes, cookDate, now = new Date()) {
