...
function computeTomorrowPreview(plan, recipes, today) {
    const todayIdx = plan.days.findIndex(d => d.date === today);
    if (todayIdx === -1 || todayIdx >= plan.days.length - 1)
        return undefined;
...
function missingByCriticality(recipe, missingItems) {
 ...
 */
export function computeTonightState(plan, recipes, missingItems, substitutions, today, currentTime) {
    const day = resolveDay(plan, today);
    if (!day) {
        const fallbackDayOfWeek = 'Mon';
        return {
            planId: plan.id,
    ...
    const todaysMissing = missingItems.filter(m => m.planId === plan.id && m.affectsTonight);
    ...
    const preflightStatus = dinner.preflightStatus ?? 'NONE_REQUIRED';
    let status = 'READY';
    let primaryMessage = "You're all set for tonight.";
    let secondaryMessage;
    const actions = { ...baseActions, canStartCooking: true };
    if (core.length > 0) {
