...
function nextShoppingItemId() {
    shoppingItemCounter += 1;
    return `si_${shoppingItemCounter}`;
}
/**
 ...
 */
export function buildShoppingList(plan, recipes, household) {
    const recipeById = new Map();
    for (const r of recipes)
        recipeById.set(r.id, r);
    const acc = new Map();
    for (const day of plan.days) {
        if (!day.dinner || day.dinner.outEating)
            continue;
        ...
        for (const ing of recipe.ingredients) {
            ...
            if (ing.amount === undefined ||
                ing.amount === null ||
                Number.isNaN(ing.amount)) {
                throw new Error(`Ingredient amount is invalid for ingredientId=${ing.ingredientId} in recipe=${recipe.id}`);
            }
            if (!ing.displayName) {
                throw new Error(`Ingredient displayName is missing for ingredientId=${ing.ingredientId} in recipe=${recipe.id}`);
            }
    ...
    const quickReviewCandidates = [];
    for (const item of items) {
        if (item.criticality === 'CRITICAL')
            continue;
        const stapleCategory = item.shoppingCategory === 'PANTRY_DRY' ||
