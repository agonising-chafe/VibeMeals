// Generated by scripts/spoonacular-workflow.ts on 2025-12-11T21:45:23.880Z
// Source: promoted from src/dev/imported (provenance preserved there)
// ⚠️  This is a canonical recipe file. Edit manually or regenerate with workflow.

import { Recipe } from '../types';

export const scrambledEggsWithAsparagusSmokedSalmonAndChives = {
  id: "atk_scrambled-eggs-with-asparagus-smoked-salmon-and-chives",
  name: "Scrambled Eggs with Asparagus, Smoked Salmon, and Chives",
  slug: "scrambled-eggs-with-asparagus-smoked-salmon-and-chives",
  metadata: {
    timeBand: "FAST",
    estimatedMinutes: 25,
    equipmentTags: [
      "LARGE_SKILLET"
    ],
    leftoverStrategy: "EXPECTED",
    baseServings: 4,
    yieldText: "SERVES 4"
  },
  scalable: true,
  ingredients: [
    {
          ingredientId: "large-eggs",
          displayName: "large eggs",
          amount: 8,
          unit: "UNIT",
          criticality: "CRITICAL",
          kind: "PROTEIN",
          shoppingCategory: "DAIRY_EGGS",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "extra-virgin-olive-oil-divided",
          displayName: "extra-virgin olive oil, divided",
          amount: 3,
          unit: "TBSP",
          criticality: "CRITICAL",
          kind: "FAT_OIL",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "minced-fresh-chives-divided",
          displayName: "minced fresh chives, divided",
          amount: 2,
          unit: "TBSP",
          criticality: "NON_CRITICAL",
          kind: "VEG",
          shoppingCategory: "PRODUCE",
          quantityKind: "FIXED",
          component: "GARNISH"
        },
    {
          ingredientId: "table-salt",
          displayName: "table salt",
          amount: 0.25,
          unit: "TSP",
          criticality: "CRITICAL",
          kind: "CONDIMENT",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "TO_TASTE"
        },
    {
          ingredientId: "pepper",
          displayName: "pepper",
          amount: 0.25,
          unit: "TSP",
          criticality: "CRITICAL",
          kind: "CONDIMENT",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "TO_TASTE"
        },
    {
          ingredientId: "clove-minced",
          displayName: "clove, minced",
          amount: 1,
          unit: "GRAM",
          criticality: "NON_CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PRODUCE",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "thin-asparagus-trimmed-and-cut-into-1-2-inch-lengths",
          displayName: "thin asparagus, trimmed and cut into 1/2-inch lengths",
          amount: 8,
          unit: "OZ",
          criticality: "CRITICAL",
          kind: "VEG",
          shoppingCategory: "PRODUCE",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "water",
          displayName: "water",
          amount: 2,
          unit: "TBSP",
          criticality: "NON_CRITICAL",
          kind: "OTHER",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "smoked-salmon-torn-into-1-2-inch-strips",
          displayName: "smoked salmon, torn into 1/2-inch strips",
          amount: 2,
          unit: "OZ",
          criticality: "CRITICAL",
          kind: "PROTEIN",
          shoppingCategory: "MEAT_SEAFOOD",
          quantityKind: "FIXED",
          allergens: [
            "FISH"
          ]
        }
  ],
  preflight: [],
  steps: [
    {
          stepNumber: 1,
          instruction: "In medium bowl, beat eggs, 2 tablespoons oil, 1 tablespoon chives, salt, and pepper with fork until no streaks of white remain. Heat 1 teaspoon oil and garlic in 12-inch nonstick skillet over medium heat until fragrant, about 1 minute. Add asparagus and water, cover, and cook, stirring occasionally, until asparagus is crisp-tender, 3 to 4 minutes. Uncover and continue to cook until moisture has evaporated, about 1 minute longer. Transfer asparagus mixture to small bowl and set aside. Wipe skillet clean with paper towels."
        },
    {
          stepNumber: 2,
          instruction: "Heat remaining 2 teaspoons oil in now-empty skillet over medium-high heat until shimmering. Add egg mixture and, using rubber spatula, constantly and firmly scrape along bottom and sides of skillet until eggs begin to clump and spatula just leaves trail on bottom of skillet, 30 to 60 seconds. Reduce heat to low and gently but constantly fold eggs until clumped and just slightly wet, 30 to 60 seconds. Fold in asparagus mixture. Transfer to serving dish, top with salmon, sprinkle with remaining 1 tablespoon chives, and serve."
        }
  ],
  tags: [
    "atk_source",
    "american",
    "comfort_food"
  ],
  recipeAllergens: [
    "FISH"
  ]
} satisfies Recipe;
