// Generated by scripts/spoonacular-workflow.ts on 2025-12-11T21:45:18.394Z
// Source: promoted from src/dev/imported (provenance preserved there)
// ⚠️  This is a canonical recipe file. Edit manually or regenerate with workflow.

import { Recipe } from '../types';

export const chickenFriedSteaks = {
  id: "atk_chicken-fried-steaks",
  name: "Chicken-Fried Steaks",
  slug: "chicken-fried-steaks",
  metadata: {
    timeBand: "NORMAL",
    estimatedMinutes: 75,
    equipmentTags: [
      "DUTCH_OVEN"
    ],
    leftoverStrategy: "NONE",
    baseServings: 6,
    yieldText: "SERVES 6"
  },
  scalable: true,
  ingredients: [
    {
          ingredientId: "unbleached-all-purpose-flour",
          displayName: "unbleached all-purpose flour",
          amount: 3,
          unit: "CUP",
          criticality: "CRITICAL",
          kind: "CARB",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "table-salt-and-ground-black-pepper",
          displayName: "Table salt and ground black pepper",
          amount: 1,
          unit: "UNIT",
          criticality: "CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "TO_TASTE"
        },
    {
          ingredientId: "cayenne-pepper",
          displayName: "cayenne pepper",
          amount: 0.125,
          unit: "TSP",
          criticality: "NON_CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "large-egg",
          displayName: "large egg",
          amount: 1,
          unit: "UNIT",
          criticality: "CRITICAL",
          kind: "DAIRY",
          shoppingCategory: "DAIRY_EGGS",
          quantityKind: "FIXED",
          allergens: [
            "EGG"
          ]
        },
    {
          ingredientId: "baking-powder",
          displayName: "baking powder",
          amount: 1,
          unit: "TSP",
          criticality: "CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "baking-soda",
          displayName: "baking soda",
          amount: 0.5,
          unit: "TSP",
          criticality: "CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "buttermilk",
          displayName: "buttermilk",
          amount: 1,
          unit: "CUP",
          criticality: "CRITICAL",
          kind: "DAIRY",
          shoppingCategory: "DAIRY_EGGS",
          quantityKind: "FIXED",
          allergens: [
            "DAIRY"
          ]
        },
    {
          ingredientId: "cube-steaks-pounded-1-3-inch-thick",
          displayName: "cube steaks, pounded 1/3 inch thick",
          amount: 6,
          unit: "OZ",
          criticality: "CRITICAL",
          kind: "PROTEIN",
          shoppingCategory: "MEAT_SEAFOOD",
          quantityKind: "FIXED"
        },
    {
          ingredientId: "peanut-oil",
          displayName: "peanut oil",
          amount: 45,
          unit: "CUP",
          criticality: "CRITICAL",
          kind: "FAT_OIL",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED",
          allergens: [
            "PEANUT",
            "TREE_NUT"
          ]
        },
    {
          ingredientId: "medium-onion-minced",
          displayName: "medium onion, minced",
          amount: 1,
          unit: "UNIT",
          criticality: "CRITICAL",
          kind: "VEG",
          shoppingCategory: "PRODUCE",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "dried-thyme",
          displayName: "dried thyme",
          amount: 0.125,
          unit: "TSP",
          criticality: "NON_CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "medium-garlic-cloves-minced-or-pressed-through-a-garlic-press-about-2-teaspoons",
          displayName: "medium garlic cloves, minced or pressed through a garlic press (about 2 teaspoons)",
          amount: 2,
          unit: "UNIT",
          criticality: "CRITICAL",
          kind: "VEG",
          shoppingCategory: "PRODUCE",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "unbleached-all-purpose-flour",
          displayName: "unbleached all-purpose flour",
          amount: 3,
          unit: "TBSP",
          criticality: "CRITICAL",
          kind: "CARB",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "low-sodium-chicken-broth",
          displayName: "low-sodium chicken broth",
          amount: 0.5,
          unit: "CUP",
          criticality: "CRITICAL",
          kind: "OTHER",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "whole-milk",
          displayName: "whole milk",
          amount: 2,
          unit: "CUP",
          criticality: "CRITICAL",
          kind: "DAIRY",
          shoppingCategory: "DAIRY_EGGS",
          quantityKind: "FIXED",
          component: "GRAVY",
          allergens: [
            "DAIRY"
          ]
        },
    {
          ingredientId: "table-salt",
          displayName: "table salt",
          amount: 0.75,
          unit: "TSP",
          criticality: "CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "ground-black-pepper",
          displayName: "ground black pepper",
          amount: 0.25,
          unit: "TSP",
          criticality: "NON_CRITICAL",
          kind: "SPICE",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED",
          component: "GRAVY"
        },
    {
          ingredientId: "pinch-cayenne-pepper",
          displayName: "Pinch cayenne pepper",
          amount: 1,
          unit: "UNIT",
          criticality: "CRITICAL",
          kind: "OTHER",
          shoppingCategory: "PANTRY_DRY",
          quantityKind: "FIXED"
        }
  ],
  preflight: [],
  steps: [
    {
          stepNumber: 1,
          instruction: "FOR THE STEAKS: Mix the flour, 5 teaspoons salt, 1 teaspoon black pepper, and the cayenne together in a large shallow dish. In a second large shallow dish, beat the egg, baking powder, and baking soda; stir in the buttermilk."
        },
    {
          stepNumber: 2,
          instruction: "Set a wire rack over a large rimmed baking sheet. Pat the steaks dry with paper towels and sprinkle each side with salt and pepper. One at a time, drop the steaks into the flour and shake the dish to coat. Shake excess flour from each steak, then, using tongs, dip each steak into the egg mixture, turning to coat well and allowing the excess to drip off. Coat the steaks with flour again, shake off the excess, and place them on the wire rack."
        },
    {
          stepNumber: 3,
          instruction: "Adjust an oven rack to the middle position, set a second wire rack over a second rimmed baking sheet, and place the sheet on the oven rack; heat the oven to 200 degrees. Line a large plate with a double layer of paper towels. Meanwhile, heat 1 inch of oil in a large (11-inch diameter) Dutch oven over medium-high heat to 375 degrees. Place 3 steaks in the oil and fry, turning once, until deep golden brown on each side, about 5 minutes (the oil temperature will drop to around 335 degrees). Transfer the steaks to the paper towel–lined plate to drain, then transfer them to the wire rack in the oven. Bring the oil back to 375 degrees and repeat the cooking and draining process (use fresh paper towels) with the 3 remaining steaks."
        },
    {
          stepNumber: 4,
          instruction: "FOR THE GRAVY: Carefully pour the hot oil through a fine-mesh strainer into a clean pot. Return the browned bits from the strainer along with 2 tablespoons of the frying oil to the Dutch oven. Turn the heat to medium, add the onion and thyme, and cook until the onion has softened and is beginning to brown, 4 to 5 minutes. Add the garlic and cook until aromatic, about 30 seconds. Add the flour to the pan and stir until well combined and starting to dissolve, about 1 minute. Whisk in the broth, scraping any browned bits off the bottom of the pan. Whisk in the milk, salt, black pepper, and cayenne; bring to a simmer over medium-high heat. Cook until thickened (the gravy should have a loose consistency—it will thicken as it cools), about 5 minutes."
        },
    {
          stepNumber: 5,
          instruction: "Transfer the chicken-fried steaks to individual plates. Spoon a generous amount of gravy over each steak. Serve immediately, passing any remaining gravy separately."
        }
  ],
  tags: [
    "atk_source",
    "american",
    "comfort_food"
  ],
  recipeAllergens: [
    "EGG",
    "DAIRY",
    "PEANUT",
    "TREE_NUT"
  ]
} satisfies Recipe;
